...
QEMU_BASE="-machine virt -cpu cortex-a57 -m 2048M -smp 1 -nographic -kernel $KERNEL -drive if=pflash,file=$PROJECT_DIR/flash.img,format=raw,index=1 -netdev user,id=net0,hostfwd=udp::9000-:9000 -device virtio-net-device,netdev=net0"
QEMU_DOCKER_FLAGS="-machine virt -cpu cortex-a57 -m 2048M -smp 1 -nographic -kernel /workspace/build/kernel.elf -drive if=pflash,file=/workspace/flash.img,format=raw,index=1 -netdev user,id=net0,hostfwd=udp::9000-:9000 -device virtio-net-device,netdev=net0"
