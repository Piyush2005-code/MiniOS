...
QEMU_BASE="-machine virt -cpu cortex-a53 -m 512M -nographic -kernel $KERNEL -drive if=pflash,file=$PROJECT_DIR/flash.img,format=raw,index=1"
QEMU_DOCKER_FLAGS="-machine virt -cpu cortex-a53 -m 512M -nographic -kernel /workspace/build/kernel.elf -drive if=pflash,file=/workspace/flash.img,format=raw,index=1"
