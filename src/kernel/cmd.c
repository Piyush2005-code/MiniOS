...
void CMD_Dispatch(char *line)
{
    ...
    while (*line && cmd_isspace(*line)) line++;
    if (*line == '\0') return;   /* blank line — do nothing */
    ...
    while (*p != '\0' && argc < CMD_MAX_ARGS) {
        /* Skip whitespace */
        while (*p && cmd_isspace(*p)) { *p = '\0'; p++; }
        if (*p == '\0') break;

        argv[argc++] = p;

        /* Advance to end of token */
        while (*p && !cmd_isspace(*p)) p++;
    }
...
static void print_wall_time(uint64_t secs)
{
    ...
    if (h < 10) { HAL_UART_PutString("0"); } HAL_UART_PutDec(h);
    HAL_UART_PutString(":");
    if (m < 10) { HAL_UART_PutString("0"); } HAL_UART_PutDec(m);
    HAL_UART_PutString(":");
    if (s < 10) { HAL_UART_PutString("0"); } HAL_UART_PutDec(s);
}
...
static void cmd_help(int argc, char *argv[])
{
    ...
    for (uint32_t i = 0; i < g_cmd_count; i++) {
        ...
        while (n[len]) len++;
        for (uint32_t sp = len; sp < 10; sp++) HAL_UART_PutString(" ");
...
static void cmd_memstat(int argc, char *argv[])
{
    ...
    HAL_UART_PutDec((uint32_t)(st.heap_used  / 1024U)); HAL_UART_PutString(" KB used\n");
    HAL_UART_PutString("         ");
    HAL_UART_PutDec((uint32_t)(st.heap_total / 1024U)); HAL_UART_PutString(" KB total\n");
    HAL_UART_PutString("         ");
